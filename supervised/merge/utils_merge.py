...
def read_data_csv(folder_name, file_name):
    ...
    # # Aramis' addition, uncomment above lines and comment the below lines (except return) if they work for you
    # data = np.genfromtxt(os.path.join(os.getcwd(), folder_name, file_name), delimiter=',', skip_header=1, dtype=None)
    # print("loaded {} datas!".format(data.shape[0])) 
    # # print("Shape of data:", data.shape) # Add this line to check the shape of the loaded array
    # data = data.reshape(-1, 1) # Reshape the array to have two dimensions
